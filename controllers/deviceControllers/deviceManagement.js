...
// Register device with system information
...
export const updateDeviceHealth = async (req, res) => {
  try {
    ...
    } else if ((metrics.cpu_usage < 50 && metrics.memory_usage < 80) || internet_status === true) {
...
export const getDeviceHealth = async (req, res) => {
  try {
    ...
    const stats = {
      ...
      avg_cpu: healthData.reduce((sum, record) => sum + (record.metrics.cpu_usage || 0), 0) / healthData.length,
      avg_memory: healthData.reduce((sum, record) => sum + (record.metrics.memory_usage || 0), 0) / healthData.length,
      online_percentage: (healthData.filter(record => record.metrics.internet_status === true).length / healthData.length) * 100
    };
...
// Get health overview for all devices
export const getAllDevicesHealth = async (req, res) => {
  try {
    ...
    const devices = await Rpi.find(filter)
      .select('rpi_id rpi_name rpi_status location last_seen device_info app_version config wifi_ssid wifi_password')
      .sort({ last_seen: -1 })
    ...
    const deviceHealth = await Promise.all(
      devices.map(async (device) => {
        ...
        const isOffline = Date.now() - new Date(device.last_seen).getTime() > 15 * 60 * 1000;
    ...
    const withWifi = deviceHealth.filter(d => d.has_wifi_configuration).length;
  ...
  try {
    ...
    res.status(200).json({
      ...
      device: {
        ...
        is_offline: Date.now() - new Date(device.last_seen).getTime() > 15 * 60 * 1000,
